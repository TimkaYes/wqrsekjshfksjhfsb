...
local rname = ""
local charet = "qwertyuiopasfhjklznm1234567890-_+=<>.,?!"

for i = 1, 15 do
	local randomIndex = math.random(1, #charet)
	rname = rname .. string.sub(charet, randomIndex, randomIndex)
end


SimpleSpy2.Name = rname
...
for i = 1, 15 do
	local randomIndex = math.random(1, #charet)
	rtame = rtame .. string.sub(charet, randomIndex, randomIndex)
end

RemoteTemplate.Name = rtame
...
local layoutOrderNum = 9999999999
...
_G.SIMPLESPYCONFIG_MaxRemotes = 350
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY TEST\n\n"
				.. logs[#logs].GenScript
		end
	end)
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.SimpleSpyMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.TestMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in Test:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
			and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.TestMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("Test: namecall method not found!")
				onToggleButtonClick()
				return
			end
...
if not _G.SimpleSpyExecuted then
	...
	if not succeeded then
		warn(
			"A fatal error has occured, Test was unable to launch properly.\nPlease DM this error message to @exx#9394:\n\n"
				.. tostring(err)
		)
		SimpleSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the Test serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the Test Serializer."
	end
end)
...
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes Test ignore it, but it will continue to be usable."
end, function()
	if selected then
...
newButton("Block (i)", function()
	return "Click to stop this remote from firing.\nBlocking a remote won't remove it from Test logs, but it will not continue to fire the server."
end, function()
	if selected then
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up Test to detection and/or instability.",
